  ...
  return (
    <div>
      ...
      <div className='absolute top-1/3 sm:top-1/4 left-5 right-5 sm:left-10 max-w-lg md:max-w-2xl'>
        <h1 className='py-3 truncate italic text-white text-3xl md:text-5xl'>{movie?.title || movie?.name || movie?.original_name}</h1>
        <p className='text-white text-base md:text-xl'>
          {truncate(movie?.overview!, 200)}
        </p>
        <div className='flex justify-start gap-x-10 mt-3'>   
          <button className='bannerBtn'><PlayIcon className='bannerIcon'/> Play</button>
          <button className='bannerBtn'><InformationCircleIcon className='bannerIcon' /> More Info</button>
        </div>
      </div>
    </div>
  )
