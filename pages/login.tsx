...
const Login = () => {
  ...
  const signIn = async (e: React.MouseEvent<HTMLElement>) => {
    ...
    if (emailRef && pwRef) {
      try {
        await signInWithEmailAndPassword(auth, emailRef.current!.value, pwRef.current!.value)
        await router.push("/")
      } catch (error: any) {
      ...
      <form className='flex justify-center'>
        <div className='w-80 sm:w-96 absolute top-40 rounded-md bg-black/80 flex flex-col items-center py-12 gap-y-8'>
          ...
          <div className='gap-y-4 flex flex-col items-center'>
            ...
            <input ref={pwRef} type="password" placeholder='Password' className='input' />
            {/* {pw.length < 4 || pw.length > 60 && <p className='error'>Your password must contain between <br /> 4 and 60 characters.</p>} */}
            
            <button onClick={signIn} className='mt-5 bg-[#e50914] font-bold w-64 py-3 rounded-md text-white' type='submit'>
              Sign In
            </button>
